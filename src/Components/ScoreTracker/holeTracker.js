    ...
    {props.roundLength.map((hole)=> (
      <div style={{display: props.activeIndex === hole ? 'block' : 'none'}} key={hole}>
          ...
          {state.activePlayers.map((player, index) => {
                        return <PlayerScoreComponent name={player} key={index} hole={hole} scorecard={props.activeScorecard} setScore={(playerName, score)=> {updateScore(playerName, score, hole)}}/>;
                      })
                }
          </div>
      </div>))}
